...
function activate(context) {
    ...
    collector = new signals_1.SignalCollector(context);
    // 2. Start JSONL logger (writes every 5 seconds)
    logger = new logger_1.SessionLogger(collector, context);
    // 3. Register confusion label shortcut
    labeler = new labels_1.LabelManager(collector, logger, context);
    const labelCommand = vscode.commands.registerCommand("unstuckai.label", () => labeler.handleLabel());
    ...
    if (isFirstRun) {
        vscode.window.showInformationMessage("UnstuckAI is now collecting your coding behaviour. " +
            "Press Ctrl+Shift+C (or Cmd+Shift+C on Mac) whenever you feel confused. " +
            "The more you label, the better the model gets.", "Got it");
        context.globalState.update("unstuckai.firstRun", false);
    }
    // 6. Register stats command (shows how much data collected)
    const statsCommand = vscode.commands.registerCommand("unstuckai.stats", () => {
        const stats = logger.getStats();
        vscode.window.showInformationMessage(`UnstuckAI Stats — ` +
    ...
    context.subscriptions.push(labelCommand, statsCommand, statusBar, { dispose: () => collector?.dispose() });
