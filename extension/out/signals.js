...
class SignalCollector {
    constructor(context) {
        ...
        this.terminalBuffer = ""; // accumulates terminal output lines
        this.lastTerminalError = 0; // timestamp of last terminal error
    ...
    attachEditorListeners() {
        ...
        const onTextChange = vscode.workspace.onDidChangeTextDocument(event => {
            ...
            for (const change of event.contentChanges) {
                ...
                const { errorCount: pErrCount, errorType: pErrType } = this.getProblemsErrors(event.document.uri);
    ...
    watchTerminalCreate() {
        ...
        if ("onDidStartTerminalShellExecution" in vscode.window) {
            const onExec = vscode.window.onDidStartTerminalShellExecution((event) => {
        ...
        for (const line of lines) {
            ...
            for (const pattern of PYTHON_ERROR_PATTERNS) {
                if (pattern.regex.test(cleanLine)) {
                    ...
                    console.log(`[UnstuckAI] Terminal error detected: ${pattern.type}`, `line=${this.terminalErrorLine}`, `count=${this.terminalErrorCount}`);
                    // Show subtle status bar indicator
                    vscode.window.setStatusBarMessage(`$(error) UnstuckAI detected: ${pattern.type} — press Ctrl+Shift+C if confused`, 5000);
                    break;
                }
            }
        }
    ...
    extractErrorType(message) {
        ...
        const knownMatch = message.match(/^([A-Za-z][A-Za-z0-9_]+(Error|Exception|Warning)):/);
        if (knownMatch) {
            return knownMatch[1];
        }
        ...
        if (/Expected \d+ argument/.test(message)) {
            return "TSWrongArgCount";
        }
